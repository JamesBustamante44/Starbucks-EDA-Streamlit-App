...
def load_dataset(use_builtin=True):
    options = []
    if use_builtin:
        for fname in ['cleaned_starbucks.csv', 'starbucks.csv']:
            if os.path.exists(fname):
                options.append(fname)
    uploaded = st.file_uploader('Upload a Starbucks CSV (optional)', type=['csv'])
    if uploaded is not None:
        try:
            df = pd.read_csv(uploaded)
            return df, 'uploaded'
        except Exception as e:
            st.warning('Could not read uploaded file. Falling back to built-ins if available.')
    if len(options) > 0:
        choice = st.selectbox('Choose a built-in dataset', options)
        df = pd.read_csv(choice)
        return df, choice
    st.info('No dataset found. Using a small synthetic sample so the app can render.')
    data = {
        'Beverage': ['Caffe Latte','Cappuccino','Mocha','Cold Brew','Americano','Espresso'],
        'Category': ['Espresso','Espresso','Espresso','Cold','Espresso','Espresso'],
        'Calories': [190,120,290,5,15,5],
        'Sugars (g)': [18,10,35,0,0,0],
        'Fat (g)': [7,4,13,0,0,0],
        'Protein (g)': [12,8,9,0,1,1],
        'Caffeine (mg)': [150,150,175,200,150,150]
    }
